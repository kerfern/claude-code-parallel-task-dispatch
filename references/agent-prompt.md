...
STEP 1 — ANALYZE
Read and understand the task before writing code.
...
- Factual errors in task → report, do NOT silently reinterpret
...
- If you need read-only file changes, describe in report

STEP 5 — TEST
5a. Run relevant tests BEFORE changes (baseline)
5b. Run tests AFTER changes
5c. New regressions > 0 → fix. If unfixable → status: failed
